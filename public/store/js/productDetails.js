	    	        	 ...
	    	        	 $(data).each(function( index, value ) {
	    	        	    if ( value.item_id == id[1]){
				    	        ...
				    	        if(parseInt(product.item_price) > 0){
				    	        	$('#id_buyNowParent').append('<a href="#" onClick="onGoShoppingCartMethods('+product.item_id+')" class="btn-round"><i class="icon-basket-loaded margin-right-5"></i>Buy Now</a>');
...
function onGoShoppingCartMethods(id){
	var currentUser = JSON.parse(localStorage.getItem('currentUser'));
	if(currentUser == null || currentUser == 'undefined' || currentUser == ''){
		  window.location = "LoginForm.html?redirect=1&product="+id+"";
	}else{
		  window.location = 'ShoppingCart.html?product='+id+'';
	}
}
